...
function randomSid(length: number): string {
 ...
 */
export function canonicalizeHandCashPaymentRequestUrl(paymentRequestUrl: string): string {
  let u: URL;
  try {
  ...
  if (u.hostname !== LEGACY_PAY_HOST) return paymentRequestUrl;

  const segments = u.pathname.split("/").filter(Boolean);
  if (segments.length !== 1) return paymentRequestUrl;
  const id = segments[0]!;
  if (!TEMPLATE_ID_RE.test(id)) return paymentRequestUrl;
